...
namespace Vision_Align
{
    public partial class FormModel : Form
    {
        ...
        private void SetUi(Matching_Type type)
        {
            string strTitle = "";
        ...
        public void ShowViewer(int nCamNo, Matching_Type type)
        {
            //this.TopMost = true;
            m_nCamNo = nCamNo;
        ...
        private void btn_Test_Click(object sender, EventArgs e)
        {
            ...
            modelParam.nNumLevels = Convert.ToInt32(nud_MG_NumLevel.Value);
            modelParam.dAngleMin = Convert.ToDouble(nud_MG_Angle_Min.Value);
            modelParam.dAngleMax = Convert.ToDouble(nud_MG_Angle_Max.Value);
            modelParam.dMinScore = Convert.ToDouble(nud_MG_MinScore.Value);
            modelParam.dMaxOverlap = Convert.ToDouble(nud_MG_MaxOverlap.Value);
            modelParam.strSubPixel = cb_MG_SubPixel.Items[cb_MG_SubPixel.SelectedIndex].ToString();

            bool bRslt = Global.clsAlgorithm[m_nCamNo].NCC_FindModel(modelParam, Global.hModelInfo[m_nCamNo, (int)matching_Type], out ModelRslt rslt);
            
            Global.clsAlgorithm[m_nCamNo].OverlayNccModel(bRslt, hv_WindowHandle, rslt.dX, rslt.dY, rslt.dAngle, rslt.dScore, "cyan");
        }
        #endregion
        ...
        private void btn_Add_Mask_Click(object sender, EventArgs e)
        {
            ...
            shape.dRadius       = Convert.ToDouble(nud_MS_Radius.Value);
            shape.strBackGround = cb_MS_BackGround.Items[cb_MS_BackGround.SelectedIndex].ToString();

            Global.clsAlgorithm[m_nCamNo].CreateCircleImg(hv_WindowHandle, shape);
        }
        ...
        {
            ModelParam shape = new ModelParam();
            shape.dSizeX = Convert.ToDouble(nud_GS_X_Size.Value);
            shape.dSizeY = Convert.ToDouble(nud_GS_Y_Size.Value);
            shape.dDepth = Convert.ToDouble(nud_GS_Depth.Value);
            shape.strBackGround = cb_MS_BackGround.Items[cb_GS_BackGround.SelectedIndex].ToString();

            Global.clsAlgorithm[m_nCamNo].CreateCrossImg(hv_WindowHandle, shape);
        }
        ...
        private void SetModel(string strModel)
        {
            ...
            switch (matching_Type)
            {
                case Matching_Type.MARK_GRAY:
                    ...
                    modelParam.dAngleMin = Convert.ToDouble(nud_MG_Angle_Min.Value);
                    modelParam.dAngleMax = Convert.ToDouble(nud_MG_Angle_Max.Value);
                    modelParam.strMetric = cb_GrayMetric.Items[cb_GrayMetric.SelectedIndex].ToString();
                ...
                case Matching_Type.MASK_SHAPE:
                    modelParam.nNumLevels = Convert.ToInt32(nud_MS_NumLevel.Value);
                    modelParam.dAngleMin  = Convert.ToDouble(nud_MS_Angle_Min.Value);
                    modelParam.dAngleMax  = Convert.ToDouble(nud_MS_Angle_Max.Value);
                    modelParam.strMetric  = cb_MS_Metric.Items[cb_MS_Metric.SelectedIndex].ToString();
                    modelParam.dRadius    = Convert.ToDouble(nud_MS_Radius.Value);
                    Global.clsAlgorithm[m_nCamNo].CreateShmMaskModel(hv_WindowHandle, modelParam, ref Global.hModelInfo[m_nCamNo, (int)matching_Type]);

                    //Global.clsAlgorithm[m_nCamNo].SaveModel(matching_Type, Global.hModelInfo[m_nCamNo, (int)matching_Type], strPath + "\\" + strModel);
                    break;

                case Matching_Type.GLASS_SHAPE:
                    modelParam.nNumLevels = Convert.ToInt32(nud_GS_NumLevel.Value);
                    modelParam.dAngleMin = Convert.ToDouble(nud_GS_Angle_Min.Value);
                    modelParam.dAngleMax = Convert.ToDouble(nud_GS_Angle_Max.Value);
                    modelParam.strMetric = cb_GS_Metric.Items[cb_GS_Metric.SelectedIndex].ToString();
                    modelParam.dSizeX = Convert.ToDouble(nud_GS_X_Size.Value);
                    modelParam.dSizeY = Convert.ToDouble(nud_GS_Y_Size.Value);

                    Global.clsAlgorithm[m_nCamNo].CreateShmGlassModel(hv_WindowHandle, modelParam, ref Global.hModelInfo[m_nCamNo, (int)matching_Type]);
                    //Global.clsAlgorithm[m_nCamNo].SaveModel(matching_Type, Global.hModelInfo[m_nCamNo, (int)matching_Type], strPath + "\\" + strModel);
        ...
        private void GetFolder(string strPath)
        {
            try
            {
                ...
                if (matching_Type == Matching_Type.GLASS_SHAPE || matching_Type == Matching_Type.MASK_SHAPE) strExtension = ".shm"; 
        ...
        private void IsLoadModel(bool IsLoad, string strModel)
        {
            ...
            switch (matching_Type)
            {
                case Matching_Type.MARK_GRAY  :  break;
                case Matching_Type.MASK_SHAPE :  break;
                case Matching_Type.GLASS_SHAPE:  break;
            }
        }
        ...
        private void IsLoadParam(bool IsLoad, string strModel)
        {
            switch (matching_Type)
            {
                case Matching_Type.MARK_GRAY  : IsLoadMaskG (IsLoad, strModel); break;
                case Matching_Type.MASK_SHAPE : IsLoadMaskS (IsLoad, strModel); break;
                case Matching_Type.GLASS_SHAPE: IsLoadGlassS(IsLoad, strModel); break;
            }
            string strPath = string.Format("{0}\\{1}\\{2}", m_strTypeFolder, strModel, strModel);
            if(IsLoad) Global.clsAlgorithm[m_nCamNo].SetImage(hv_WindowHandle, strPath);
        }
        ...
        private void IsLoadMaskS(bool IsLoad, string strModel)
        {
            try
            {
                ...
                if (IsLoad)
                {
                    ...
                    nud_MS_Boader    .Value = (decimal)Global.recipeMask_Shm[m_nCamNo].dBoader     ;
                    nud_MS_Pixel     .Value = (decimal)Global.recipeMask_Shm[m_nCamNo].dPixel      ;
                ...
                else
                {
                    ...
                    Global.recipeMask_Shm[m_nCamNo].dBoader      = Convert.ToDouble(nud_MS_Boader    .Value);
                    Global.recipeMask_Shm[m_nCamNo].dPixel       = Convert.ToDouble(nud_MS_Pixel     .Value);
        ...
        private void IsLoadGlassS(bool IsLoad, string strModel)
        {
            try
            {
                ...
                if (IsLoad)
                {
                    ...
                    nud_GS_Boader     .Value = (decimal)Global.recipeGlass_Shm[m_nCamNo].dBoader     ;
                    nud_GS_Pixel      .Value = (decimal)Global.recipeGlass_Shm[m_nCamNo].dPixel      ;
                ...
                else
                {
                    ...
                    Global.recipeGlass_Shm[m_nCamNo].dBoader      = Convert.ToDouble(nud_GS_Boader     .Value);
                    Global.recipeGlass_Shm[m_nCamNo].dPixel       = Convert.ToDouble(nud_GS_Pixel      .Value);
